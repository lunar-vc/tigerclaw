...
Daily Gmail triage for LP prospects. Cross-references a parsed LP index against recent Gmail activity to surface emails needing response, prioritized by tier (Existing LP > Warm > Cold).

**Location:** `scripts/lp-triage.js` (triage), `scripts/parse-lp-csv.js` (one-time CSV parser)
**Data:** `.lp-index.json` (project root, generated by parse-lp-csv.js)
**Requires:** Gmail Monitor skill (gmail-monitor)
...
**What it does:**
1. Loads `.lp-index.json` (197 LPs across ~40 email domains)
2. Batches Gmail queries by domain (~15 domains per query, ~8 queries total)
3. Cross-references results against LP index using domain and address lookups
4. Classifies: **needs_response** (inbound, in INBOX) vs **recent_activity** (outbound or read)
5. Sorts by tier priority (existing_lp > warm > cold), then recency

**Output:**
- `NEEDS RESPONSE` — inbound emails from LP contacts sitting in INBOX
- `RECENT ACTIVITY` — outbound emails or already-read/archived threads

**When to use:** When Mick says "check LP emails", "LP triage", or "any LP emails". NOT added to session startup — invoked on demand only. Re-run `parse-lp-csv.js` if the source CSV is updated.
...
3. **Search every stack layer** — not just direct competitors. For any thesis, explicitly search:
   - **Core layer:** Direct competitors doing the same thing
   - **Upstream:** Data providers, hardware suppliers, infrastructure they depend on
   - **Downstream:** Customers, integrators, distribution channels
   - **Adjacent tooling:** MLOps, deployment platforms, dev tools, evaluation/benchmarking tools that serve the same customer
   - **Geographic variants:** Same thesis in EU, US, Israel, Asia — different teams, same problem
4. **Check European VC portfolios for recent announcements.** Run `node scripts/vc-portfolio-scan.js --domain=<domain>` to surface pre-seed/seed companies from European VCs that may not appear in generic web searches
5. Find white space and underserved segments
6. Assess market timing signals
7. Save to `research/YYYY-MM-DD-market-slug.md`
8. Create Linear issue in **THE** team (Triage) if a new theme emerges from the mapping
...
Four additional scan scripts beyond the core latent-founder-signals search:

**`scripts/vc-portfolio-scan.js`** — European VC portfolio and startup press monitoring
```bash
node scripts/vc-portfolio-scan.js --domain=robotics       # Robotics domain
...
```
Searches European VC blogs (Speedinvest, Cherry, Earlybird, Seedcamp, FOV Ventures, Atomico, etc.), startup media (Sifted, EU-Startups, Breakit, Nordic9), accelerator announcements (EF, Antler), and "startups to watch" lists. Catches pre-seed/seed companies that only appear in niche European outlets and are invisible to generic web searches. **Run this during every market landscape mapping and competitive landscape analysis.**
